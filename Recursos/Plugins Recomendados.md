...
## Configuração exata recomendada

Use estes valores nas configurações do plugin **Spaced Repetition**:

| Configuração | Valor |
|---|---|
| Etiquetas para revisar | `#review` |
| Separador para flashcards inline | `::` |
| Separador para flashcards inline reversos | `:::` |
| Separador para flashcards de múltiplas linhas | `?` |
| Separador para flashcards de múltiplas linhas reversos | `??` |
| Converter `==marca-texto==` em omissões | ativado |

Formato correto para cartões inline:

```md
#flashcards/javascript/strings
O que é uma string?::Um tipo de dado usado para representar texto.
Como acessar o primeiro caractere de `nome`?::`nome[0]`
JavaScript usa ==tipagem dinâmica==.
```

Atenção à nomenclatura: use sempre a tag base `#flashcards` para cartões/baralhos e `#review` somente para a fila de revisão de notas inteiras.
